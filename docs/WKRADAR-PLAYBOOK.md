...
| **`ValidoImportacao="Sim"`** | Existe nos 3 configs "personalizados" (Ocorrências/Minerador, Espelho=Apurações4, Apurações5) — **ausente** nos 2 mais antigos (Banco de Horas, D_Empregado/Informativos). É sempre o ÚLTIMO campo do arquivo, depois de todos os campos de filtro/dados — padrão típico de metadado DO PRÓPRIO config, não parâmetro de exportação. Hipótese (não 100% confirmada, sem acesso ao código do WK): selo interno que o assistente/designer de relatório do WK grava quando o relatório é salvo com sucesso pela UI ("essa definição passou pela validação"), explicando por que só existe nos 3 relatórios criados via designer, não nos 2 mais antigos/built-in. Não achamos NENHUM efeito no conteúdo exportado ao variar esse campo entre os 3 configs (todos com "Sim", todos exportando normal há meses). **Decisão (William, 2026-07-08): não mexer, sem motivo pra isso.** |
...
## Quirk: o layout de colunas do export é ditado pelo `Hash`/`Modelo` salvo no WK, NÃO pelo texto do config — copiar um config não recria o relatório (2026-07-06)

O item 3 dos quirks gerais ("`Hash` não valida conteúdo — é só identificador do modelo/relatório") é verdadeiro mas **enganoso** se lido como "edito à vontade sem me preocupar". Na prática: o `Hash`+`Modelo` de um `Config_*.txt` apontam pra uma **definição de relatório salva dentro do próprio WK** (criada/editada só pelo Modelador, na UI). Copiar um arquivo de config inteiro (texto) reproduz fielmente o comportamento **daquela definição salva** — mas se a definição em si foi **re-salva/sobrescrita** depois (outra pessoa editou o mesmo relatório no Modelador, mudando colunas/filtros), toda cópia feita a partir dali (mesmo Hash) passa a sair com o **layout novo**, não o antigo que você lembra ter visto.

**Caso real (2026-07-06):** ao tentar automatizar o export de Apurações (relatório rico de 20 colunas: Cód.Emp/Ficha/Crachá/Nome/Ocorrência/Observação/Situação/Tipo Situação/Horário/Originais/Apuradas/Previstas/Saldo/Departamento/Turma/Escala/Tipo/Motivo — gerado manualmente em 26/06 via `Config_Relatorio_de_Apurações.txt`, sem sufixo), copiei esse mesmo config (mesmo Hash) pra um novo arquivo, só trocando `ArquivoExportacao`/`ListarSomenteOcorrencias`/`IdsFuncionarios`. Rodei de verdade: exit 0, mtime avançou, arquivo gerado — mas saiu com **9 colunas** (o relatório "Relação de Ocorrências", semanal, agregado). O motivo: esse Hash tinha sido **reaproveitado** em 2026-07-01/02 (o config sem sufixo virou `WK_OCORR_CONFIG`, repontado no Modelador pro relatório enxuto de Ocorrências) — a definição salva no WK por trás daquele Hash **já não era mais** a rica. Confirmei também que o outro Hash existente na família (`..._1`, usado pelo Espelho de Ponto) resolve pra um layout DIFERENTE ainda (7 colunas) — ou seja, existem só 2 definições de relatório "Apuração-like" vivas no WK hoje, nenhuma delas a rica de 20 colunas.
