...
**FEITO (PC, 2026-07-10, v325/1.63.0, commit bcec8f3 — em paralelo a este refinamento).** Ação
`banco-horas-geral` no data.js; card `rh_confere` com `rotaBH` ganhou selo info "Resolvido no
Banco de Horas" + botão único Conferir (sem Dispensar); o Conferir da GP abre o
`openConferirAutoModal` DIRETO de `rh_confere` com a Ação pré-selecionada pela `acaoSugerida`
(trocável) e hint explicando o roteamento. **Achado que simplificou tudo: ZERO mudança de regra
e zero no firebase.js** — o ramo RH/admin da regra já aceitava `rh_confere -> confirmada` com
ação, o `_transicaoOca` não depende do estágio de origem (e ainda zera `classificacaoIncerta` ao
sair de rh_confere de graça), e o `podeConferirUI` citado no spec original é das manuais (o
fluxo auto usa delegado próprio). Resto do fluxo intocado (Lançar -> Lançadas com "Lançada ·
Banco de Horas Geral"). Smoke Playwright demo 8/8, 0 erros.

**Reconciliado:** o PC testou/deployou contra os 37 cards que existiam no momento (regra ampla,
antes do refinamento). A limpeza de Firestore (35 apagados, ver acima) aconteceu depois — estado
...
## 2026-07-20 (tarde) · 🔍 Auditoria completa: "por que isso aconteceu de novo?" — workflow de 6 agentes + 4 fixes de proteção + agendamento estendido pro fim de semana

William perguntou direto, depois de ver o 2º incidente do dia: "pq isso ACONTENCEU NOVAMENTE". Em vez de responder só com o caso pontual, rodei uma auditoria completa (workflow com 6 agentes: timeline histórica, risco em todos os exports, cobertura do circuit breaker em outros uploaders, e se o gap de fim de semana é recorrente) pra dar uma resposta honesta e decidir se tinha uma causa estrutural não fechada.

### Resposta: SIM, é a mesma família — 8 incidentes desde 03/07, escalada clara de escopo
Timeline completa (grep em todo o histórico): 8 incidentes da família "WK consolida dado assincronamente, pipeline lê antes de terminar" desde a origem (26 Faltas falsas em 03/07). Os 2 primeiros fixes (03/07, 06/07 manhã) foram PONTUAIS (ajuste de agendamento + limpeza manual). A partir de 06/07 à tarde (nascimento da reverificação contínua + circuit breaker), todo fix subsequente foi GENERALIZANTE (mecanismo reutilizável) — mas cada fonte NOVA (Espelho/999 em 16/07, Minerador/loop principal em 20/07) ainda precisou descobrir sua própria evidência específica. O princípio já está documentado como playbook; a implementação por fonte continua caso a caso.

### Mapa de risco real (não hipotético) — exposições encontradas
- **`export-empregado.mjs`/`process-empregado.mjs`**: único caminho que reúne SEM retry no export E SEM nenhum gate de frescor no consumidor (só guarda anti-vazio, não anti-velho). Alimenta `sync-colaborador-users.mjs` (liga/desliga login) — CSV desatualizado mas bem-formado passa 100% silencioso.
- **Atrasos/Saída Antecipada no loop principal**: mesmo circuit breaker pooled do incidente de hoje, ZERO evidência cruzada, 29-47% do volume do loop principal (não é caso raro). **Decisão: NÃO estender `espelhoDiaCoerente` pra esses tipos** — o próprio Fable já rejeitou essa extensão hoje mais cedo (ver seção anterior): um atraso genuíno pequeno (ex. 21min) ainda cai dentro de `JANELA_MATCH_MIN`=120min e pareceria "coerente" por engano, fechando um card REAL. Fica como risco monitorado (visível no heartbeat via `occ-auto-alerta.json`), não como bug a corrigir agora — precisa de evidência específica calibrada contra falso-positivo, não a receita já usada.
- **`upload-to-firestore.mjs`**: inativação por Demissão do CSV (linha ~132) SEM proteção proporcional (só a inativação por AUSÊNCIA, um caminho diferente, tem guarda binária). Cascateia sem proteção própria pra 2 lugares: poda do mural (`upload-aniversariantes.mjs`, zero proteção — o único delete em massa do repo sem breaker nenhum) e desativação de login (`sync-colaborador-users.mjs`, reseta senha como efeito colateral).
- **`upload-banco-horas-self.mjs`**: `catch{}` totalmente silencioso na leitura do Espelho + `batch.set()` sem merge podia zerar o campo `dias` de TODO MUNDO numa rodada só, sem log.
- **Confirmado protegido, sem ação necessária**: detector 999/Espelho, Faltas Injustificadas do Minerador, `process-espelho-ponto.mjs` (gate duplo), `upload-identificacao.mjs` (guarda binária + breaker 50% já existentes).

### Fim de semana: estrutural e recorrente, não coincidência isolada
Confirmado com dado real (`ExpAuto_Espelho_Ponto.txt`, leitura direta, sem varredura): **58-64 funcionários trabalham sábado de verdade** (turnos 1/2 reduzidos + turno 3 madrugada) — não é população marginal. O agendamento seg-sex é fixo desde a criação da tarefa (22/05), e é a **ÚNICA automação do servidor com essa restrição** — o pipeline-irmão Comercial e os backups rodam 7 dias/semana sem problema. O gatilho de fundo (WK não fechar o dia a tempo) já acontece ~2x/semana em dia útil e normalmente se autocorrige em ~19h (rodada seguinte) — o fim de semana estica essa janela pra ~67h, tempo suficiente pra virar rajada visível em vez de sumir sozinho.

### Ações tomadas (com autorização explícita do William pra cada uma)
1. **Agendamento estendido pra 7 dias** (`Export-ScheduledTask`/`Register-ScheduledTask`, XML editado adicionando `<Saturday/><Sunday/>` aos 3 `CalendarTrigger` — 09:00/11:00/14:00 continuam os mesmos horários, resto da config/Principal/Action intocado). Fecha a janela de 67h que causou o incidente de hoje; reduz também a exposição residual em Atrasos/Saída Antecipada (mesmo padrão, menor janela de acúmulo). Verificado pós-mudança: `State: Ready`, `NextRunTime` correto, XML confirma os 7 dias nos 3 triggers.
2. **`upload-to-firestore.mjs`**: breaker de 50% na inativação por Demissão — mesmo padrão dos outros 3 uploaders. Se o CSV desta rodada marcaria mais de metade dos ativos como demitido, ignora o sinal de Demissão inteiro (preserva `ativo`/`demissao` anteriores), loga aviso.
3. **`upload-aniversariantes.mjs`**: mesmo breaker de 50% na poda do mural.
4. **`upload-banco-horas-self.mjs`**: rastreia se Espelho/D_Empregado carregaram; se não, OMITE os campos `dias`/`funcionarioTurno` da escrita (em vez de gravar vazio/null) e usa `merge:true` — valor anterior sobrevive.

Todos os 3 fixes de código testados com dado real: nenhum breaker disparou hoje (sem evento de massa em curso), comportamento normal confirmado intocado (`dias`/`funcionarioTurno` populados certinho pós-fix).
...
### Deploy
Código commitado (`process-ocorrencias-rh.py`). Backfill dos docs existentes rodado via `resync-ocorrencias-horario-relevante.mjs` (dry-run primeiro, depois real **com confirmação explícita do William no chat**) — de 9 casos medidos, só 4 ainda estavam em `rh_confere` (os outros 5 já tinham sido revisados por humano, guarda existente do script não tocou neles): Jacques (476, 20/07 e 21/07), **Anderson Dobuchak (612, 22/07 — o caso motivador)**, Jenifer (671, 22/07). Só o campo `apuradasAlinhadas` mudou em cada um (dedupId intocado, nada de status/sit).

## 2026-07-23/24 · 2 missões do PC via bridge: cadência do `recemChegados` (hardening) + `saldoDiario` (correção de premissa)

Duas missões aprovadas pelo William, resgatadas pelo PC via bridge no mesmo lote. Investiguei as duas com um workflow de 2 agentes em paralelo (leitura só, sem editar nada) antes de mexer em código, pra não agir em cima de suposição.

### Missão 1 — cadência do `recemChegados` (bug reportado: admitido há 1 dia não aparecia no card de boas-vindas do colaborador)

**Não achei bug na lógica.** O filtro (`upload-aniversariantes.mjs`, `diasNaCasa >= 0 && diasNaCasa <= 15`) está correto, sem off-by-one nem problema de timezone. A tarefa agendada já roda **7 dias por semana**, 3x/dia (`DaysOfWeek=127` confirmado via `Get-ScheduledTask` — o comentário no topo de `run-pipeline.mjs` dizia "de segunda a sexta", **desatualizado desde que sábado/domingo entraram em 2026-07-15**; corrigido). Conferi ao vivo no Firestore (`config/aniversariantes`) contra a fonte viva (`funcionarios`): no momento da checagem o snapshot **já estava correto** — os 2 admitidos em 22/07 já apareciam em `recemChegados`. Ou seja, o sintoma que o PC reportou foi provavelmente transitório (checado antes da rodada seguinte rodar) e se autocorrigiu sozinho, como o desenho já prevê.

**O risco real, e o que decidi corrigir:** o passo é *best-effort* (`try/catch` só loga aviso em `run-pipeline.mjs`, sem retry nem alerta) — se ele (ou uma dependência upstream, `process-empregado.mjs`/`upload-to-firestore.mjs`) falhar silenciosamente por 1+ rodadas seguidas, o snapshot fica obsoleto sem ninguém perceber, porque é a ÚNICA fonte do card do colaborador (que não pode ler `/funcionarios` ao vivo, diferente do card do gestor). Mesma classe de risco já resolvida pros CSVs do WK (`fontesVelhas`) e pro circuit breaker de ocorrências (`occAutoBreakerAbortado`) — apliquei o mesmo padrão aqui: `upload-aniversariantes.mjs` grava um marcador de último sucesso (`aniversariantes-ultimo-sucesso.json`, mtime é o sinal, gitignored — mesma família do `occ-auto-alerta.json`); `write-heartbeat-report.mjs` lê a idade desse arquivo e acende `recemChegadosObsoleto` no frontmatter + uma seção nova no report quando passa de **20h** sem sucesso (margem antes de estourar a promessa de "atraso máximo 1 dia" pro William). Testado ao vivo: rodei os 2 scripts de verdade, marcador gravado, report renderizou a seção nova certinha (`✅ Atualizado há 0.0h`), e testei a lógica do limiar isoladamente com um mtime forjado de 25h pra confirmar que a branch de alerta ativa.

### Missão 2 — `saldoDiario` (pedido: "verifica se foi implementado; se não, alimenta")

**A missão partia de uma premissa errada — a feature já existe, só que sob outro nome, e já está ligada ponta a ponta.** Não existe (nem deveria) um campo literal `saldoDiario` no fluxo do Espelho — esse nome só aparece no fluxo, totalmente separado, de Ocorrências (`o.saldoDiario`, de `ExpAuto_Apuracoes.txt`). O que o PC provavelmente lembrava como "pendência da era v289-v291" **já foi entregue em 2026-07-14** (achado do William na tela do Espelho, caso Jenifer/GP) sob os nomes `saldoDiaFmt` (saldo nativo do WK, cumulativo, multiplicado pela situação) + `saldoDiaOriginalMin`/`saldoDiaOriginalFmt` (equivalente sem multiplicador, cruzado com `parsed-bh.json`) — calculados em `process-espelho-ponto.mjs`, subidos por `upload-banco-horas-self.mjs` em `banco-horas-self/{codigo}.dias[]`, e **já consumidos pelo front**: `app.js:9034` (`saldoDiaStr`, prefere o Original) usado em `app.js:5096` (Meu Ponto do colaborador) e `app.js:5997` (Espelho do gestor). Conferi eu mesmo os 3 pontos no código antes de reportar — bate exatamente com o que o agente de investigação achou.
