...
- **Achado lateral**: `IdsFuncionarios` no `Config_Banco_de_Horas.txt` apareceu como lista FIXA de 148 ids (não mais vazio/dinâmico) — provável efeito colateral de mexer na tela do relatório hoje. William falou que isso já foi resolvido antes (funcionário novo entra normal), mas o dado ATUAL do config contradiz — vale ele conferir/limpar a aba "Seleção" da próxima vez que abrir esse relatório no WK.
